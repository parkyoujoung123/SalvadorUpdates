...
bot.command("pullupdate", async ctx => {
  ...
  try {
    ...
    await ctx.reply("✅ Update berhasil divalidasi. Bot akan restart dalam 2 detik.");
    setTimeout(() => process.exit(0), 2000);
  } catch (error) {
    fs.rmSync(UPDATE_TEMP_PATH, { force: true });
    console.error("Auto-update gagal:", error);
    await ctx.reply(`❌ Update dibatalkan. File lama tetap aman.\\n\\nError: ${String(error.message || error).slice(0, 400)}`);
...
bot.use(async (ctx, next) => {
    try {
        ...
        if (!joined) {
            ...
            await ctx.replyWithPhoto(thumbnailUrl, {
                caption: `<blockquote><strong>( 🔒 ) Kamu wajib join channel dulu sebelum menggunakan bot ini.</strong></blockquote>`,
                parse_mode: 'HTML',
                reply_markup: {
                    inline_keyboard: [
                        [
...
bot.action('check_join', async (ctx) => {
    try {
        ...
        if (joined) {
            ...
            await ctx.answerCbQuery('❌ Kamu belum join channel! Silakan join dulu.', { show_alert: true });
        }
    } catch (e) {
...
☇ Platform : Telegram
☇ Version : 1.0 [ New Release ]
☇ Type Script : Free Spam & Not Spam
  ...
  const keyboard = [
    ...
    [
      ...
      },
      {
        ...
        url: "https://t.me/Informationsalvador",
        style: "primary",
      },
    ],
  ];
