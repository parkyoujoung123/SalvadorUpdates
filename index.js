...
☇ Connection : ${senderStatus}
☇ Runtime : ${runtimeStatus}
<blockquote><strong><tg-emoji emoji-id="528284376445">⚙️</tg-emoji> 𝖢𝖮𝖭𝖳𝖱𝖮𝖫 𝖢𝖤𝖭𝖳𝖤𝖱</strong></blockquote>
...
`;
  ...
  try {
    // Telegram dapat menolak caption jika tag HTML/custom emoji tidak valid.
    // Menu CENTER dikirim sebagai teks biasa agar selalu aman diparse.
    const safeControlsMenu = controlsMenu.replace(/<[^>]*>/g, "");
    await ctx.editMessageCaption(safeControlsMenu, {
      reply_markup: {
        inline_keyboard: keyboard,
      },
    });
    ...
    if (
      ...
      console.error("Gagal membuka menu CENTER:", error?.response?.description || error.message);
      await ctx.answerCbQuery("Menu CENTER gagal dibuka").catch(() => {});
    }
  }
});
...
☇ Connection : ${senderStatus}
☇ Runtime : ${runtimeStatus}
<blockquote><strong><tg-emoji emoji-id="5271604874419647061">🔗</tg-emoji> 𝖳𝖮𝖮𝖫𝖲 𝖬𝖤𝖭𝖴</strong></blockquote>
